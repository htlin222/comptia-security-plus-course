  ...
  const cites = (ev.citations || []).length
    ? `<div class="Evidence__cite">
         ${ev.citations
           .map(
             (c) =>
               `<a href="${esc(c.url)}" target="_blank" rel="noopener" title="${esc(c.title)}">${esc(citeLabel(c) || c.title)}</a>`,
           )
           .join("")}
       </div>`
    : "";
...
/* 引用的識別碼可能是 PMID、NIST 編號、RFC、ATT&CK technique 或 CVE。
   連結一律用資料裡的 url，框架不替任何來源組網址——組錯的話會連到別份文件，
   而且看起來完全正常。 */
/* "RFC 8446" 的 id 已經帶了來源前綴，再加一次 type 會變成 "RFC RFC 8446"。
   NIST 的 id 則是 "SP 800-207"，前綴要補上才看得出是誰家的文件。 */
function citeLabel(c) {
  const type = c.type ? String(c.type).toUpperCase() : "";
  const id = String(c.id || "");
  if (!type) return id;
  if (!id) return type;
  return id.toUpperCase().startsWith(type) ? id : `${type} ${id}`;
}

function citeLi(c) {
  const label = citeLabel(c);
  const meta = [c.publisher || c.journal, c.year, label].filter(Boolean).join(" · ");
  const name = esc(c.title || c.id || "");
  return `
    <li>
      ${c.url ? `<a href="${esc(c.url)}" target="_blank" rel="noopener">${name}</a>` : name}
      ${meta ? `<span class="DrillEvCat__src">${esc(meta)}</span>` : ""}
  ...
  return `
    <section class="Evidence DrillEv" data-drillev="${esc(u.id)}">
      <button class="Evidence__header" type="button" data-toggle="drillev">
        ...
        <span class="Evidence__spacer"></span>
        ${UI.evidenceSource ? `<span class="Label Label--neutral">${esc(UI.evidenceSource)}</span>` : ""}
        <span class="Evidence__chevron">${icon("chevron-right", 14)}</span>
      </button>
...
/* 單元可以掛一份外部筆記（unit.note）。影片負責講懂，筆記負責講細——
   兩邊分開放但互相指得到，比把筆記塞進影片描述有用得多。
   欄位全部選用，缺哪個就不渲染哪個，框架不預設筆記長什麼樣。 */
function noteBox(n) {
  if (!n || !n.url) return "";

  const link = (href, iconName, label) =>
    href
      ? `<a class="Note__link" href="${esc(href)}" target="_blank" rel="noopener">
           ${icon(iconName, 12)} ${esc(label)} ${icon("external-link", 10)}
         </a>`
      : "";

  const concepts = (n.concepts || []).length
    ? `<div class="Note__concepts">
         ${n.concepts.map((c) => `<span class="Label Label--neutral">${esc(c)}</span>`).join("")}
       </div>`
    : "";
  ...
  return `
    <section class="Note">
      <header class="Note__head">
        ${icon("notebook-pen", 14)}
        <span class="Note__label">${esc(UI.noteLabel || "")}</span>
        <span class="Note__title">${esc(n.title || "")}</span>
      </header>
      ${
        n.eli5
          ? `<p class="Note__eli5"><span class="Note__eli5Label">${esc(UI.eli5Label || "")}</span>${esc(n.eli5)}</p>`
          : ""
      }
      ${concepts}
      <div class="Note__links">
        ${link(n.url, "book-open", UI.noteLinkLabel || "")}
        ${link(n.caseUrl, "file-text", UI.noteCaseLabel || "")}
        ${n.questions ? link(n.url + "#practice-questions", "list-checks", `${n.questions} ${UI.noteQuestionsLabel || ""}`) : ""}
      </div>
    </section>`;
}
...
export function renderUnit(u, done, locked = false) {
  ...
  const badges = [
    ...
    // 高權重主題標在標題列，考前要挑重點看時不必一個個展開
    u.note?.weight === "high"
      ? `<span class="Label Label--attention" title="${esc(UI.highWeightHint || "")}">${icon("flame", 11)} ${esc(UI.highWeightLabel || "high")}</span>`
      : "",
    u.note?.questions
      ? `<span class="Label Label--neutral" title="${esc(UI.noteQuestionsLabel || "")}">${icon("list-checks", 11)} ${u.note.questions}</span>`
      : "",
    ...
    <div class="PaywallGate">
      ...
      <span class="PaywallGate__main">
        ...
        <p class="PaywallGate__note">
          ${unitTotal}${esc(UI.unitNoun || " 個單元")}${drillTotal ? ` · ${drillTotal}${esc(UI.drillNoun || "")}` : ""}
