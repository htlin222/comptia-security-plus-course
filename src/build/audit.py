...
# 引用識別碼的格式規則。這裡只做離線的格式檢查——「這個編號長得對不對」；
# 「這份文件是不是真的存在」由 verify_refs.py 打真實 API 決定，兩件事別混為一談。
CITATION_ID = {
    "pmid": re.compile(r"^\d{6,9}$"),
    "doi": re.compile(r"^10\.\d{4,9}/\S+$"),
    "nist": re.compile(r"^(SP \d{3}-\d+[A-Za-z]?( Part \d+)?( Rev\. \d+)?|FIPS \d+(-\d+)?|CSWP \d+)$"),
    "rfc": re.compile(r"^RFC \d{1,5}$"),
    "attck": re.compile(r"^T\d{4}(\.\d{3})?$"),
    "cve": re.compile(r"^CVE-\d{4}-\d{4,}$"),
}


def bad_citation(c: dict) -> str | None:
    """回傳不合格的原因字串；合格回 None。"""
    if not isinstance(c, dict):
        return "引用不是物件"
    # 舊資料只寫 pmid 沒寫 type，補推一次免得整批被判不合格
    ctype = c.get("type") or ("pmid" if c.get("pmid") else None)
    if ctype not in CITATION_ID:
        return f"type={c.get('type')!r} 不在 {'／'.join(CITATION_ID)}"
    ident = str(c.get("id") or c.get(ctype) or "").strip()
    if not CITATION_ID[ctype].match(ident):
        return f"{ctype}={ident!r} 格式不合"
    if ctype != "pmid" and not str(c.get("url") or "").startswith("https://"):
        return f"{ctype}:{ident} 缺 url（程式不替任何來源組網址）"
    return None
...
def audit_depth(cfg: dict, units: list[dict], opts: dict, rep: Report) -> None:
    ...
    for pattern, key in (("unit-evidence-*.json", "topics"), ("oe-*.json", "conditions")):
        for path in sorted(DATA.glob(pattern)):
            blob = load_json(path)
            for cond in (blob or {}).get(key, []) if isinstance(blob, dict) else []:
    ...
    for path in sorted(DATA.glob("drill-evidence-*.json")):
        ...
        for cat in (blob or {}).get("categories", []) if isinstance(blob, dict) else []:
            ...
            for c in cites:
                if bad := bad_citation(c):
                    bad_pmid.append(f"{cat['id']} · {bad} · {(c.get('title') or '')[:40]}")
    ...
    if bad_pmid:
        rep.err(
            sec,
            f"{len(bad_pmid)} 筆引用的識別碼格式不合法（跑 make verify 打真實 API 才算數）",
            bad_pmid,
        )
    if thin_cats:
