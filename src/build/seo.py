...
def _sprite_symbol(name: str) -> str:
    """從 build_icons 打包好的 sprite 取出單一圖示，還原成獨立 SVG。

    og.html 是給 headless Chrome 截圖用的靜態頁，載不了 JS 模組，所以圖示得在
    建置時就內嵌進去。這樣品牌圖示永遠跟著 config 的 brandIcon 走，不會留下
    上一門課的符號——那是換主題最常被忘記的一格。
    """
    js = (ROOT / "src" / "web" / "js" / "icons.js").read_text()
    # sprite 是 JS 字串字面值，引號都被跳脫過，直接對原始碼下正則會撲空
    lit = re.search(r'export const ICON_SPRITE =\s*(".*?");', js, re.S)
    sprite = json.loads(lit.group(1)) if lit else ""
    m = re.search(rf'<symbol id="i-{re.escape(name)}"[^>]*>(.*?)</symbol>', sprite, re.S)
    inner = m.group(1) if m else ""
    if not inner:
        print(f"   ⚠ og.html 找不到圖示 {name}，確認它在 build_icons.py 的 ICONS 裡")
    return (
        '<svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" '
        f'stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">{inner}</svg>'
    )


def render_og(cfg: dict, meta: dict) -> None:
    tpl_path = ROOT / "src" / "web" / "og.html"
    if not tpl_path.exists():
        return

    og = cfg.get("og") or {}
    stats = "".join(
        f'<div class="stat"><b>{meta.get(s["field"], "")}</b><span>{s["label"]}</span></div>'
        for s in (cfg.get("ui", {}).get("stats") or [])[: og.get("statCount", 4)]
    )
    grades = "".join(
        f'<span class="chip c-{g.get("tone", "neutral")}">{g["label"]}</span>'
        for g in (cfg.get("grades") or [])
    )
    host = re.sub(r"^https?://|/$", "", cfg["site"]["url"])

    out = tpl_path.read_text()
    for key, val in {
        "brandIcon": _sprite_symbol(cfg["site"].get("brandIcon", "activity")),
        "evidenceIcon": _sprite_symbol(og.get("evidenceIcon", "microscope")),
        "site.name": cfg["site"]["name"],
        "site.host": host,
        "hero.heading": og.get("heading") or cfg.get("hero", {}).get("heading", ""),
        "hero.lede": og.get("lede") or cfg.get("hero", {}).get("lede", ""),
        "og.evidenceNote": og.get("evidenceNote", ""),
        "stats": stats,
        "grades": grades,
    }.items():
        out = out.replace("{{" + key + "}}", str(val))

    (PUB / "og.html").write_text(out)
    print("   og.html    已依設定檔生成（make og 會截這一份）")
