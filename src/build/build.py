...
# 課程可以自帶詞彙模組（分面與項目分類）；沒有就退化成無分面、無分類
...
def notes_meta() -> dict:
    ...
    return {f"notes_{k}": v for k, v in counts.items()}


def collect_evidence() -> dict:
    """把單元層級的實證檔攤平成 unit_id -> evidence dict。

    檔名與節點名有兩套：`unit-evidence-*.json` / `topics` 是現在的寫法，
    `oe-*.json` / `conditions` 是早期課程留下的，兩種都讀，換主題時不必改資料。
    """
    ev = {}
...
def main() -> int:
    ...
    for code, title, ic, src in CHAPTERS:
        ...
        if got_drills != want_drills:
            problems.append(f"{code}: 項目 {got_drills} 個，應為 {want_drills}")
        ...
        for u in units:
            ...
            # 單元層級的分面：單元名稱與兩組標籤都算涉及
            u["facets"] = facets.extract(u.get("name"), *(u.get("tight") or []), *(u.get("weak") or [])) if facets else []
    ...
    # 分面索引：依群組分類，數量少的不進篩選器（避免上百個一次性標籤）
    muscle_index = [
        {
            "name": m,
    ...
    print(f"   主課單元 {unit_total} · 項目 {drill_total} · 合計 {unit_total + drill_total}")
    ...
    if uncategorised:
        print(
            f"   ⚠ 未分類項目 {len(uncategorised)}：{'、'.join(x for x in uncategorised[:5] if x)}"
        )
